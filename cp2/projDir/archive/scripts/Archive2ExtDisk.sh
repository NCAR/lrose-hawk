...
started_in=$EXloc
testARCMD="$E [test] rsync -av"
realARCMD="rsync -av"
ARCMD=$realARCMD
testARCMDQUIET="$E [test] rsync -a"
realARCMDQUIET="rsync -a"
...
readDiskUsage() {
	...
	#    useDiskLabel	  text
	#    useFirstArchived  yyyymmdd
	#    useLastArchived  yyyymmdd
	#    useLastWrite     yyyymmdd_hhmm.ss
	...
	then
		...
		then
			...
			useFirstArchived=`grep "FirstArchived:" $usagefile | perl -e '{ @line=split ( /\s+/,<>); print "@line[1]"; }' -`
			useFirstWrite=`grep "FirstWrite:" $usagefile | perl -e '{ @line=split ( /\s+/,<>); print "@line[1]"; }' -`
			useLastArchived=`grep "LastArchived:" $usagefile | perl -e '{ @line=split ( /\s+/,<>); print "@line[1]"; }' -`
			useLastWrite=`grep "LastWrite:" $usagefile | perl -e '{ @line=split ( /\s+/,<>); print "@line[1]"; }' -`
	...
	tmpSize=`/bin/df | grep $diskroottail | perl -e '{ @line=split ( /\s+/,<>); print "@line[1]"; }' -`
	if [ -z "$useDiskSize" ]
	then
	...
	tmpSize=`/bin/df | grep $diskroottail | perl -e '{ @line=split ( /\s+/,<>); print "@line[3]"; }' -`
	if [ -z "$useSpaceLeft" ]
	then
...
writeDiskUsage() {
	...
	[ "$useStatus" = "WRITING" ] && startedWrite=`/bin/date +%Y%m%d_%H%M.%S`
	...
	then
		...
		if [ $useThisDay -gt 0 ]
		then
			[ -z "$useFirstArchived" ] && useFirstArchived=$curdate
			[[ $curdate -lt $useFirstArchived ]] && useFirstArchived=$curdate
			[ -z "$useFirstWrite" ] && [ ! -z "$startedWrite" ] && useFirstWrite=$startedWrite
			[ -z "$useLastArchived" ] && useLastArchived=$curdate
			[[ $curdate -gt $useLastArchived ]] && useLastArchived=$curdate
			useLastWrite=`/bin/date +%Y%m%d_%H%M.%S`
			useLastWrote=$useThisDay
		fi 
		[ ! -z "$useFirstArchived" ] && $E "FirstArchived:\t$useFirstArchived" >> $usagefile
		[ ! -z "$useFirstWrite" ] && $E "FirstWrite:\t$useFirstWrite" >> $usagefile
		[ ! -z "$useLastArchived" ] && $E "LastArchived:\t$useLastArchived" >> $usagefile
		[ ! -z "$useLastWrite" ] && $E "LastWrite:\t$useLastWrite" >> $usagefile
				...
				case $command in
				   ...
				   real )
						...
						then
							...
							read reply
							[ "$reply" != "YES" ] && ERROR_EXIT "Script Exited..."
						fi
				   ;;
				   test )
						ARCMD=$testARCMD
...
[ $del_mode ] && diskroot=/media/usbdisk
...
diskavailstart=`/bin/df | grep $diskroottail | perl  -e '{ @line=split ( /\s+/,<>); print "@line[3]"; }' -`
diskavailstartgb=`/bin/df -h | grep $diskroottail | perl  -e '{ @line=split ( /\s+/,<>); print "@line[3]"; }' -`
...
then
	...
	[ ! -e $logfile ] && /bin/touch $logfile && $ERROR_EXIT "Can't Create logfile: $logfile"
	[ ! -e $errorfile ] && /bin/touch $errorfile && $ERROR_EXIT "Can't Create errorfile: $errorfile"
	[ ! -e $usagefile ] && /bin/touch $usagefile && $ERROR_EXIT "Can't Create usagefile: $usagefile"
	...
	exec 1>>$logfile || { ERROR "Unable to open $logfile for log output" && exit 1; }
...
OkToWrite=true
[ $curdate -gt $enddate ] && OkToWrite=false
[ ! "$OkToWrite" = true ] && $E "Date Range At/Past End Date: $curdate > $enddate"
[ -z "$useThisDay" ] && useThisDay=0
[ -z "$MaxUsePerDay" ] && MaxUsePerDay=0
useSpaceLeft=`/bin/df | grep $diskroottail | perl -e '{ @line=split ( /\s+/,<>); print "@line[3]"; }' -`
[ $useMaxPerDay -ne 0 ] && [ $useSpaceLeft -lt $useMaxPerDay ] && OkToWrite=false
[ ! "$OkToWrite" = true ] && $E "Disk Usage: NO SPACE ON DISK. Usage Per Day = $useMaxPerDay, Space on Disk = $useSpaceLeft"

while [ "$OkToWrite" = true ]
do
	$E
		...
		case $dir in
		  calibration )
			...
			do
				...
				[ $del_mode ] && [ -d $dataroot/$dir/$subdir ] && $E "  ***** We Do Not DELETE Calibration Data *****"
			done
			;;
		  logs ) 
		  ...
		  sunscan )
			...
			do
				$E " '->>>>>>>>> Sub-Directory: $dir/$subdir/$curdate/ >>>>>>>>>>"
				[ $del_mode ] && $E "   ***** We Do Not DELETE sunscan data *****"
	...
	if ( [ "$OkToWrite" = true ] && [ $curdate -gt $enddate ] )
	then
		OkToWrite=false
