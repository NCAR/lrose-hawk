#!/usr/bin/perl
#
# FTP transparent radar image files to the field catalog
...
if ($debug) {
    print STDERR "Running handle_images_catalog_transparent:\n";
...
$field_name = "UNKNOWN";
$platform_name = "SPOL_TRANSPARENT";
if ($file_name =~ /RADAR_DBZ_F_150km/) {
    $field_name = "sband_DBZ_2deg";
} elsif ($file_name =~ /RADAR_VEL_F_150km/) {
    $field_name = "sband_VEL_2deg";
} elsif ($file_name =~ /RADAR_PID_150km/) {
    $field_name = "sband_PID_2deg";
} else {
