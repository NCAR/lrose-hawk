#!/usr/bin/perl
#
# FTP sat image files to the field catalog
#
...
if ($debug) {
    print STDERR "Running handle_images_catalog_sat:\n";
...
if ($file_name =~ /VIS_LARGE/) {
  $platform_name = "LTG_LARGE";
  $field_name = "ltg_sat_VIS";
} elsif ($file_name =~ /VIS_MEDIUM/) {
  $platform_name = "LTG_MEDIUM";
  $field_name = "ltg_sat_VIS";
} elsif ($file_name =~ /VIS_FOUR_CORNERS/) {
  $platform_name = "LTG_4CORNERS";
  $field_name = "ltg_sat_VIS";
} elsif ($file_name =~ /IR_LARGE/) {
  $platform_name = "LTG_LARGE";
  $field_name = "ltg_sat_IR";
} elsif ($file_name =~ /IR_MEDIUM/) {
  $platform_name = "LTG_MEDIUM";
  $field_name = "ltg_sat_IR";
} elsif ($file_name =~ /IR_FOUR_CORNERS/) {
  $platform_name = "LTG_4CORNERS";
  $field_name = "ltg_sat_IR";
} elsif ($file_name =~ /WV_LARGE/) {
  $platform_name = "LTG_LARGE";
  $field_name = "ltg_sat_WV";
} elsif ($file_name =~ /WV_MEDIUM/) {
  $platform_name = "LTG_MEDIUM";
  $field_name = "ltg_sat_WV";
} elsif ($file_name =~ /WV_FOUR_CORNERS/) {
  $platform_name = "LTG_4CORNERS";
  $field_name = "ltg_sat_WV";
} else {
