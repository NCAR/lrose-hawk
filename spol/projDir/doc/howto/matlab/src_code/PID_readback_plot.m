function PID_readback_plot( myInfile );
...
for kk=class_lim;
    ...
    title([ mytime(1,:) ' to ' mytime(size(mytime,1),:) ' SPolKa SUR Vols' ] ,'FontWeight','bold', 'FontSize', 14);
...
% limit the scale for the top plot (clip the NoEcho category)
    ...
    xlabel('NCAR Particle Type'); ylabel('Radar Volume-weighted Percent');
    title([ 'Spol-Ka Time change of Echo Type ' mytime(1,:) ' to ' mytime(size(mytime,1),:) ] ,'FontWeight','bold', 'FontSize', 14);
end;
