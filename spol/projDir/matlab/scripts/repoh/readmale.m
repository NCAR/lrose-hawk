...
%Author:  S. Powell, UW
%Description:  Read in Male soundings and plot data.

clear all; close all; clc;
Rstar = 287;
maxheight = 20;

system('ls -C1 ~/spowell/humidity_retrieval/male/data/*.nc > out');
filenames = [];
fid = fopen('out');
[A,B] = system('wc -l out');
...
%male_sounding.D20111002_180000.nc
%male_sounding.D20111011_180000.nc
%male_sounding.D20111020_060000.nc
%male_sounding.D20111031_120000.nc
...
for j = 1:size(filenames,2)
  ...
  if length(cell2mat(filenames(j))) ~= 91  %Bad file
    P(~isnan(P)) = NaN;
    T(~isnan(T)) = NaN;
    Td(~isnan(Td)) = NaN;
    rh(~isnan(rh)) = NaN;
...
save('~/spowell/humidity_retrieval/male/retarded.mat')
