function PID_RHI_fraction_batch(myDate);

%  Modified for batch operation.  Restrictions to output filename and file selection.
%  Requires a subdirectory: /home/operator/matlab/batch_PID 
%
...
direc = '/scr/pgen2/cfradial/partrain/sband/rhi';
%direc = '/export/d1/dynamo/data/cfradial/partrain/sband/rhi';
set(0,'DefaultTextInterpreter','none');

global rt_direc;
%rt_direc = '/export/d1/dynamo/data/cfradial';
...
trop_height = 18.0;  % approximated troposphere height 
...
ofile = ['/home/operator/projDir/data/matlab/analysis/PID_RHI_summary.' myDate '.out' ];
%ofile = ['/scr/sci/rilling/matlab/spol_dynamo/output/PID_RHI_summary.' myDate '.out'];
...
f_name = get_sorted_file_list(myDate,'partrain','sband',{'rhi'});
...
% need a sanity check to ensure that there are multiple elevations, and that the 
% fixed angle increases monotonically.  We don't want volumes with multiples
% of the same tilt.

% we don't much care about azimuth and elevation, but we want to make sure
...
%           sum(totgate), numel(ptmp));
   ...
   angs = sprintf('%6.1f',SWP.fixed_angle);
